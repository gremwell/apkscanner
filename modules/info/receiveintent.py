...
class Module(framework.module):
    ...
    def module_run(self, verbose=False):
        ...
        z = self.apk.vm_analysis.tainted_packages.search_methods(".", "getIntent", ".")
        ...
        for p in z:
            ...
            if self.apk.get_package() in method.get_class_name().replace("/", "."):
                ...
                #TODO: backtrace the register value if index is a variable
                matches = re.findall(r'getStringExtra\(([^\)]*)\);', source)
                if len(matches):
                ...
                matches = re.findall(r'getStringArrayExtra\(([^\)]*)\);', source)
                if len(matches):
                    if method.get_class_name()[1:-1] not in [x["file"] for x in results]:
                        results.append({
                            "file": method.get_class_name()[1:-1],
                ...
                matches = re.findall(r'getBooleanExtra\(([^\)]*)\);', source)
                if len(matches):
                    if method.get_class_name()[1:-1] not in [x["file"] for x in results]:
                        results.append({
                            "file": method.get_class_name()[1:-1],
                ...
                matches = re.findall(r'getBooleanArrayExtra\(([^\)]*)\);', source)
                if len(matches):
                    if method.get_class_name()[1:-1] not in [x["file"] for x in results]:
                        results.append({
                            "file": method.get_class_name()[1:-1],
                ...
                matches = re.findall(r'getCharExtra\(([^\)]*)\);', source)
                if len(matches):
                    if method.get_class_name()[1:-1] not in [x["file"] for x in results]:
                        results.append({
                            "file": method.get_class_name()[1:-1],
                ...
                matches = re.findall(r'getCharArrayExtra\(([^\)]*)\);', source)
                if len(matches):
                    if method.get_class_name()[1:-1] not in [x["file"] for x in results]:
                        results.append({
                            "file": method.get_class_name()[1:-1],
                ...
                matches = re.findall(r'getDoubleExtra\(([^\)]*)\);', source)
                if len(matches):
                    if method.get_class_name()[1:-1] not in [x["file"] for x in results]:
                        results.append({
                            "file": method.get_class_name()[1:-1],
                ...
                matches = re.findall(r'getDoubleArrayExtra\(([^\)]*)\);', source)
                if len(matches):
                    if method.get_class_name()[1:-1] not in [x["file"] for x in results]:
                        results.append({
                            "file": method.get_class_name()[1:-1],
                ...
                matches = re.findall(r'getIntExtra\(([^\)]*)\);', source)
                if len(matches):
                    if method.get_class_name()[1:-1] not in [x["file"] for x in results]:
                        results.append({
                            "file": method.get_class_name()[1:-1],
                ...
                matches = re.findall(r'getIntArrayExtra\(([^\)]*)\);', source)
                if len(matches):
                    if method.get_class_name()[1:-1] not in [x["file"] for x in results]:
                        results.append({
                            "file": method.get_class_name()[1:-1],
                ...
                matches = re.findall(r'getFloatExtra\(([^\)]*)\);', source)
                if len(matches):
                    if method.get_class_name()[1:-1] not in [x["file"] for x in results]:
                        results.append({
                            "file": method.get_class_name()[1:-1],
                ...
                matches = re.findall(r'getFloatArrayExtra\(([^\)]*)\);', source)
                if len(matches):
                    if method.get_class_name()[1:-1] not in [x["file"] for x in results]:
                        results.append({
                            "file": method.get_class_name()[1:-1],
                ...
                matches = re.findall(r'getLongExtra\(([^\)]*)\);', source)
                if len(matches):
                    if method.get_class_name()[1:-1] not in [x["file"] for x in results]:
                        results.append({
                            "file": method.get_class_name()[1:-1],
                ...
                matches = re.findall(r'getLongArrayExtra\(([^\)]*)\);', source)
                if len(matches):
                    if method.get_class_name()[1:-1] not in [x["file"] for x in results]:
                        results.append({
                            "file": method.get_class_name()[1:-1],
                ...
                matches = re.findall(r'getShortExtra\(([^\)]*)\);', source)
                if len(matches):
                    if method.get_class_name()[1:-1] not in [x["file"] for x in results]:
                        results.append({
                            "file": method.get_class_name()[1:-1],
                ...
                matches = re.findall(r'getShortArrayExtra\(([^\)]*)\);', source)
                if len(matches):
                    if method.get_class_name()[1:-1] not in [x["file"] for x in results]:
                        results.append({
                            "file": method.get_class_name()[1:-1],
                ...
                matches = re.findall(r'getURIExtra\(([^\)]*)\);', source)
                if len(matches):
                    if method.get_class_name()[1:-1] not in [x["file"] for x in results]:
                        results.append({
                            "file": method.get_class_name()[1:-1],
        ...
        if self.avd is not None:
            for r in results:
                classactivity = ".%s" % r["file"].split("/")[-1]
                for a in activities:
                    if a["name"] == classactivity and a["exported"] is True:
                        cmd = "am start -n \"%s/.%s\" -a \"android.intent.action.SEND\"" % (
                            self.apk.get_package(),
                            r["file"].split("/")[-1]
                        )
                        #TODO: content generation algorithm
                        for key in r["keys"]:
                            cmd += " --es \"%s\" \"%s\" " % (key, None)

                        print cmd
                        print self.avd.shell(cmd)
